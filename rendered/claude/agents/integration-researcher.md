...
description: "Use whenever a material external contract or feasibility is uncertain: API, SDK, library, CLI, scraping target, or a failing existing integration. Verifies current behavior and viable alternatives when needed, records prerequisites and blockers in an Integration Recipe, and never writes production code."
model: sonnet
effort: high
color: yellow
