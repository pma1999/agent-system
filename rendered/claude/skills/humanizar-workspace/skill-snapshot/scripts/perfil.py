...
def parrafos(txt):
    """Separa parrafos por linea en blanco y, si no hay ninguna, por salto simple.

    Un .txt exportado de Word suele traer los parrafos con un solo \\n. Sin este respaldo el
    diagnostico por parrafo colapsa el documento entero en un bloque y reporta en verde sin
    ...
    if len(lineas) <= 2:
        return ps
    media = sum(len(l.split()) for l in lineas) / float(len(lineas))
    if media >= 20:
        return lineas
    # Lineas cortas: el texto trae salto duro (copiado de un PDF, o de Word con ajuste de
    # linea), no un salto por parrafo. Tratar cada linea como parrafo da decenas de
    # "parrafos" de doce palabras y un diagnostico sin sentido. Se agrupan en bloques de
    # ~120 palabras, que es lo mas parecido a un parrafo sin marcas que hay.
    bloques, actual, n = [], [], 0
    for l in lineas:
        actual.append(l)
        n += len(l.split())
