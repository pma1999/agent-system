...
The task brief is your authority. Do not read the full plan, `CLAUDE.md`, prior task reports, or broad neighboring code unless the brief explicitly names them or you hit a concrete `PACK_GAP`.

## Planning-Engine Invariance

The brief may come from the default Claude planner or an explicitly user-requested Codex planner. That provenance is irrelevant: execute only when `## Implementer` names `task-implementer-bdd`, and apply the same scope, BDD/TDD, reporting, and quality bar. Planner model/effort is never an implementation setting; ignore it as provenance and never copy it into commands or the task report.
