...
Give a one-line reason in each `## Implementer` (constraint applied, or balance). Briefs assigned to `codex` must be fully self-contained (complete context pack, exact interfaces, no expected gaps) **and must include the dispatch profile**: select the lowest model/effort pair from the approved intelligence scale in the orchestrator skill's `references/codex-delegation.md` (it carries per-task cost and a quality/price verdict per pair) that safely clears the task's demand, and write it as `codex — <model>/<effort>; floor <score>: <one-line reason>`. Approved pairs are the scale's efficient-ladder rungs only — `gpt-5.6-luna` at any effort, `gpt-5.6-sol` at `medium`/`high`/`xhigh`/`max`; every `gpt-5.6-terra` pair and `sol`/`low` is cost-dominated (a smarter-and-cheaper rung exists) and must never be selected. The orchestrator owns final routing and may override with a logged reason.
