...
- When a broad diagnosis feeds planning, make the artifact equally consumable by the default Claude planner or an explicitly user-requested Codex planner. Planner identity/model/effort is irrelevant to the diagnosis and must not appear in fix direction.
