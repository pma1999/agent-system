...
- **Profiles:** every Codex dispatch is routed on the approved intelligence scale in the reference file — planner-assigned in the brief for planned tasks, orchestrator-assigned for unplanned work. The scale carries per-task cost: select the lowest efficient-ladder rung (luna low->max, then sol medium->max) that clears the capability floor; every terra pair and sol/low is cost-dominated — never select them. The planner role keeps its fixed `sol`/`xhigh` preset.
